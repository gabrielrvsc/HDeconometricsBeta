...
jackknife=function (X, y, lag = 4, fixed.controls = NULL) 
{
  ...
  if (length(fixed.controls) != 0) {
    fixed.aux = rbind(matrix(NA, lag, ncol(fixed)), fixed, 
                      matrix(NA, lag, ncol(fixed)))
  }
  for (j in 1:T) {
    X1 = lapply(Xaux, function(x) x[-c(j:(j + 2 * lag)), 
                                    ])
    X2 = lapply(Xaux, function(x) x[j + lag, ])
    y1 = yaux[-c(j:(j + 2 * lag))]
    y2 = yaux[j + lag]
    if (length(fixed.controls) != 0) {
      f1 = fixed.aux[-c(j:(j + 2 * lag)), ]
      f2 = fixed.aux[j + lag, ]
    }
    residos = matrix(NA, length(y1), N)
    erroprev = rep(NA, N)
    for (i in 1:N) {
      aux = Reduce("cbind", lapply(X1, function(x) x[, 
                                                     i]))
      ...
      if (length(fixed.controls) != 0) {
        ...
        cm=coef(model)
        cm[is.na(cm)]=0
        residos[, i] = y1 - cbind(1, f1, aux) %*% cm
        jprev = t(c(1, f2, aux2)) %*% cm
      }
      else {
        model = lm(y1 ~ aux)
        cm=coef(model)
        cm[is.na(cm)]=0
        residos[, i] = y1 - cbind(1, aux) %*% cm
        jprev = t(c(1, aux2)) %*% cm
      }
      erroprev[i] = y2 - jprev
    }
  ...
  optm = solve.QP(Dmat = dmat, dvec = dvec, Amat = t(amat), 
                  bvec = bvec)
  if (length(fixed.controls) != 0) {
  ...
  return(list(coef = save.coef, weights = optm$solution, type = "jackknife", 
              fixed.controls = fixed.controls, lag = lag, N = N))
}
